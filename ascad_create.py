...
def labelize(plaintexts, keys, subkey_index=2):
...
def extract_traces(traces_file, labeled_traces_file, profiling_index=[n for n in range(0, 50000)],
                   ...
                   profiling_desync=0, attack_desync=0, sub_key_index=2):
    ...
    desync_raw_traces = raw_traces[:,
                        range(min_target_point, max_target_point + 1 + max(profiling_desync, attack_desync))]
    ...
    for trace in attack_index:
        ...
        curr_trace += 1
    # Compute our labels
    labels_profiling = labelize(raw_plaintexts[profiling_index], raw_keys[profiling_index], sub_key_index)
    labels_attack = labelize(raw_plaintexts[attack_index], raw_keys[attack_index], sub_key_index)
    ...
    profiling_metadata = np.array(
        [(raw_plaintexts[n], raw_ciphertexts[n], raw_keys[n], raw_masks[n], profiling_desync_metadata[k]) for n, k in
    ...
    attack_metadata = np.array(
        [(raw_plaintexts[n], raw_ciphertexts[n], raw_keys[n], raw_masks[n], attack_desync_metadata[k]) for n, k in
...
if __name__ == "__main__":
    # Our folders
    ascad_data_folder = "/media/rico/Data/TU/thesis/data/ASCAD_data/ASCAD_databases/"
