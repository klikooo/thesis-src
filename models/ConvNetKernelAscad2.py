...
class ConvNetKernelAscad2(nn.Module):
    def __init__(self, input_shape, out_shape, kernel_size, channel_size=20):
        super(ConvNetKernelAscad2, self).__init__()
        ...
        self.fc4 = torch.nn.Linear(int(self.conv4_channels * num_features), 1000).to(device)
        self.fc5 = torch.nn.Linear(1000, 500).to(device)
    ...
    @staticmethod
    def save_name(args):
        return "{}_k{}".format(ConvNetKernelAscad2.basename(), args['kernel_size'])
    ...
    def load_model(file):
        ...
        model = ConvNetKernelAscad2(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return ConvNetKernelAscad2(out_shape=args['n_classes'],
