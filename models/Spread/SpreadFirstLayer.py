...
class SpreadFirstLayer(nn.Module):
    def __init__(self, spread_factor, input_shape, out_shape):
        super(SpreadFirstLayer, self).__init__()

        np_min = np.array([np.finfo(np.float32).min] * input_shape)
        np_max = np.array([np.finfo(np.float32).max] * input_shape)
        ...
        self.fc1 = nn.Linear(input_shape * spread_factor, 100).to(device)
        self.fc3 = nn.Linear(100, out_shape).to(device)
    ...
    def load_model(file):
        ...
        model = SpreadFirstLayer(checkpoint['sf'], input_shape=checkpoint['input_shape'],
    ...
    def name(self):
        return SpreadFirstLayer.basename()

    @staticmethod
    def basename():
        return SpreadFirstLayer.__name__

    @staticmethod
    def save_name(args):
        return "{}".format(SpreadFirstLayer.basename())

    @staticmethod
    def init(args):
        return SpreadFirstLayer(spread_factor=args['sf'], out_shape=args['n_classes'], input_shape=args['input_shape'])
