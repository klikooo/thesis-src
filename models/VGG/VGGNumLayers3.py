...
# Select max pool value, no dropout for classification block
class VGGNumLayers3(nn.Module):
    def __init__(self, input_shape, out_shape, num_layers, kernel_size, channel_size, max_pool=4):
        super(VGGNumLayers3, self).__init__()
    ...
    def name(self):
        return "{}_k{}_c{}_l{}_m{}".format(VGGNumLayers3.basename(),
                                           self.kernel_size, self.channel_size, self.num_layers,
                                           self.max_pool)

    @staticmethod
    def save_name(args):
        return "{}_k{}_c{}_l{}_m{}".format(VGGNumLayers3.basename(), args['kernel_size'],
                                           args['channel_size'], args['num_layers'], args['max_pool'])

    @staticmethod
    def basename():
        return VGGNumLayers3.__name__
    ...
    def load_model(file):
        ...
        model = VGGNumLayers3(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return VGGNumLayers3(out_shape=args['n_classes'],
