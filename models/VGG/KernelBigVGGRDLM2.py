...
class KernelBigVGGRDLM2(nn.Module):
    def __init__(self, input_shape, out_shape, kernel_size, channel_size):
        super(KernelBigVGGRDLM2, self).__init__()
        ...
        ##################
        # Next steps of BN
        self.conv5_1 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        self.conv5_2 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = int(num_features / 2)
        ...
        ##################
        # Next steps of BN
        self.conv6_1 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        self.conv6_2 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = int(num_features / 2)
        self.bn6 = nn.BatchNorm1d(num_features=self.conv5_channels).to(device)

        ##################
        # Next steps of BN
        self.conv7_1 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        self.conv7_2 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = int(num_features / 2)
        self.bn7 = nn.BatchNorm1d(num_features=self.conv5_channels).to(device)

        ##################
        # Next steps of BN
        self.conv8_1 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        self.conv8_2 = nn.Conv1d(self.conv5_channels, self.conv5_channels,
                                 kernel_size=self.kernel_size, padding=self.padding).to(device)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = num_features + 2 * self.padding - 1 * (self.kernel_size - 1)
        num_features = int(num_features / 2)
        self.bn8 = nn.BatchNorm1d(num_features=self.conv5_channels).to(device)
    ...
    def forward(self, x):
        ...
        x = self.bn2(self.mp1(F.relu(self.conv2_2(F.relu(self.conv2_1(x))))))
        x = self.bn3(self.mp1(F.relu(self.conv3_2(F.relu(self.conv3_1(x))))))
        x = self.bn4(self.mp1(F.relu(self.conv4_2(F.relu(self.conv4_1(x))))))
        x = self.bn5(self.mp1(F.relu(self.conv5_2(F.relu(self.conv5_1(x))))))
        x = self.bn6(self.mp1(F.relu(self.conv6_2(F.relu(self.conv6_1(x))))))
        x = self.bn7(self.mp1(F.relu(self.conv7_2(F.relu(self.conv7_1(x))))))
        x = self.bn8(self.mp1(F.relu(self.conv8_2(F.relu(self.conv8_1(x))))))
    ...
    def name(self):
        return "{}_k{}_c{}".format(KernelBigVGGRDLM2.basename(), self.kernel_size, self.channel_size)

    @staticmethod
    def basename():
        return KernelBigVGGRDLM2.__name__

    @staticmethod
    def save_name(args):
        return "{}_k{}_c{}".format(KernelBigVGGRDLM2.basename(), args['kernel_size'],
                                   args['channel_size'])
    ...
    def load_model(file):
        ...
        model = KernelBigVGGRDLM2(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return KernelBigVGGRDLM2(out_shape=args['n_classes'],
