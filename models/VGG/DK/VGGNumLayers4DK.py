...
class VGGNumLayers4DK(nn.Module):
    def __init__(self, input_shape, out_shape, num_layers, kernel_size, channel_size, max_pool=4):
        ...
        self.classification_block = nn.Sequential(
            torch.nn.Linear(int(channel_size * num_features) + 256, 256).to(device),
            nn.ReLU(),
    ...
    def name(self):
        return "{}_k{}_c{}_l{}_m{}".format(VGGNumLayers4DK.basename(),
                                           self.kernel_size, self.channel_size, self.num_layers,
                                           self.max_pool)

    @staticmethod
    def save_name(args):
        return "{}_k{}_c{}_l{}_m{}".format(VGGNumLayers4DK.basename(), args['kernel_size'],
                                           args['channel_size'], args['num_layers'], args['max_pool'])

    @staticmethod
    def basename():
        return VGGNumLayers4DK.__name__
    ...
    def load_model(file):
        ...
        model = VGGNumLayers4DK(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return VGGNumLayers4DK(out_shape=args['n_classes'],
