...
class VGGMasked(nn.Module):
    def __init__(self, input_shape, out_shape, num_layers, kernel_size, channel_size, max_pool=4):
        ...
        # MASKED BLOCK #
        ################
        self.masked_block = nn.Sequential(
    ...
    def name(self):
        return "{}_k{}_c{}_l{}_m{}".format(VGGMasked.basename(),
                                           self.kernel_size, self.channel_size, self.num_layers,
                                           self.max_pool)

    @staticmethod
    def save_name(args):
        return "{}_k{}_c{}_l{}_m{}".format(VGGMasked.basename(), args['kernel_size'],
                                           args['channel_size'], args['num_layers'], args['max_pool'])

    @staticmethod
    def basename():
        return VGGMasked.__name__
    ...
    def load_model(file):
        ...
        model = VGGMasked(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return VGGMasked(out_shape=args['n_classes'],
