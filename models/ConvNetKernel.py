...
class ConvNetKernel(nn.Module):
    def __init__(self, input_shape, out_shape, kernel_size, channel_size=8):
        ...
        self.conv2 = nn.Conv1d(self.conv1_channels, self.conv2_channels, kernel_size=self.kernel_size, padding=self.padding).to(device)
        ...
        self.conv3 = nn.Conv1d(self.conv2_channels, self.conv3_channels, kernel_size=self.kernel_size, padding=self.padding).to(device)
        self.bn3 = nn.BatchNorm1d(num_features=self.conv3_channels).to(device)
        self.mp3 = nn.MaxPool1d(5).to(device)
        size = int(size * 2 / 5)

        self.conv4 = nn.Conv1d(self.conv3_channels, self.conv3_channels, kernel_size=self.kernel_size, padding=self.padding).to(device)
    ...
    def name(self):
        return "{}_k{}".format(ConvNetKernel.basename(), self.kernel_size)

    @staticmethod
    def save_name(args):
        return "{}_k{}".format(ConvNetKernel.basename(), args['kernel_size'])
