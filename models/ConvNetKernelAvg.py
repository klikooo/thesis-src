...
class ConvNetKernelAvg(nn.Module):
    def __init__(self, input_shape, out_shape, kernel_size, channel_size=8):
        super(ConvNetKernelAvg, self).__init__()
        ...
        self.avg1 = nn.AvgPool1d(self.avg_pool).to(device)
        size = int(input_shape * self.conv1_channels / self.avg_pool)
        ...
        self.avg2 = nn.AvgPool1d(5).to(device)
        size = int(size * 2 / 5)
        ...
        self.avg3 = nn.AvgPool1d(5).to(device)
        size = int(size * 2 / 5)

        self.conv4 = nn.Conv1d(self.conv3_channels, self.conv3_channels, kernel_size=self.kernel_size,
                               padding=self.padding).to(device)
        self.bn4 = nn.BatchNorm1d(num_features=self.conv3_channels).to(device)
        self.avg4 = nn.AvgPool1d(5).to(device)
    ...
    def forward(self, x):
        ...
        # print('Inputs size {}'.format(inputs.size()))
        x = self.avg1(F.relu(self.bn1(self.conv1(inputs))))
        x = self.avg2(F.relu(self.bn2(self.conv2(x))))
        x = self.avg3(F.relu(self.bn3(self.conv3(x))))
        x = self.avg4(F.relu(self.bn4(self.conv4(x))))
    ...
    def name(self):
        return "{}_k{}".format(ConvNetKernelAvg.basename(), self.kernel_size)

    @staticmethod
    def save_name(args):
        return "{}_k{}".format(ConvNetKernelAvg.basename(), args['kernel_size'])

    @staticmethod
    def basename():
        return ConvNetKernelAvg.__name__
    ...
    def load_model(file):
        ...
        model = ConvNetKernelAvg(input_shape=checkpoint['input_shape'],
    ...
    @staticmethod
    def init(args):
        return ConvNetKernelAvg(out_shape=args['n_classes'],
