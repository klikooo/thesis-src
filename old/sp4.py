...
weights = {
    ...
    'out': tf.Variable(gl_initializer([100 * _sf_, n_classes])),
...
def spread_network(x):
    ...
    spread_layer = tf.reshape(result, [tf.shape(layer_1)[0], num_spreaded_features])
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % display_step == 0:
            ...
            print("Epoch:", '%04d' % (epoch + 1), "cost={:.9f}, acc {:.9f}".format(avg_cost, accuracy.eval({X: x_attack, Y: y_attack})))
