...
from old.spread_test import load_ascad
...
n_classes = 9
# n_classes = 256
traces_file = '/Data/TU/thesis/src/data/ASCAD_data/ASCAD_databases/subkeys/ASCAD_subkey_{}'.format(sub_key_index)
...
weights = {
    ...
    'w2': tf.Variable(gl_initializer([n_neurons * _sf_, n_hidden_1])),
    'out': tf.Variable(gl_initializer([n_hidden_1, n_classes])),
...
def spread_network(x):
    ...
    m_min = tf.reshape(tf.reduce_min(layer_1, axis=[1]), [tf.shape(x)[0], 1])
    m_max = tf.reshape(tf.reduce_max(layer_1, axis=[1]), [tf.shape(x)[0], 1])
    left = tf.subtract(m_max, m_min)

    x_grave = tf.multiply(tf.div(tf.subtract(layer_1, m_min), left), tf.to_float(_spread_factor))
    ...
    spread_layer = tf.reshape(nc, [tf.shape(layer_1)[0], num_spreaded_features])
...
with tf.Session() as sess:
    ...
    test_model(predictions, metadata_attack, sub_key_index, use_hw=True)
